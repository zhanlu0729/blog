...
```
### 5.1 服务降级参考指标
1. 平均响应时间：比如1秒内的5个请求平均响应时间均超过阈值,那么在接下来的一段时间窗口内对这个方法的访问都会自动熔断.
2. 异常比例：当某个方法每秒调用所获得的异常总数比例超过设定的阈值时,那么在接下来的一段时间窗口内对这个方法的访问都会自动熔断.
3. 异常数量：当某个方法每秒调用所获得的异常数量超过设定的阈值时,那么在接下来的一段时间窗口内对这个方法的访问都会自动熔断.
### 5.2 熔断策略
